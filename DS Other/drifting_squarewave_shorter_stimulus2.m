function [idList, AllCells, DSI, DSI_error, ratio, noiseratio,...
    ...
    drifting_squarewave_shorter_stimulus2(date, datafile, stimfile2)
...
A= strcat('/Users/erinzampaglione/Documents/Lab_Work/DSCells/', date, '/', datafile);
mkdir(A);
...
%        NeuronID = TROUBLE(q); % if TROUBLE is the neuron IDs
    ...
    %% Use the allspike arrays to make frequency plots for each orientation, in a new figure window - the PSTH
...
%     reduced_chi_square_binom(q) = chi_square_binom/15;
    ...
    DSI_error(q) = (2/(p_spikes+n_spikes)^2)*(n_spikes*p_spikes*(n_spikes+p_spikes))^(1/2);
     ...
     %% Other organizations of figures
    if printfigures == 2
        figure
        subplot(2,2,1)
