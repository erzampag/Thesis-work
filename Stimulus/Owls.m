...
%         keyboard
        ...
        for k = 1:300 % number of pixels before direction change
            if mod(k, 1) ~=0 % slows down speed from 1 pixel/frame
                StartPos = StartPos;
            else
                StartPos = StartPos+ NewDirection.*5;
                for j = 1 : length(StartPos) % is there an easier way to wrap?
                    if StartPos(j,1) > winRect(3)
                        StartPos(j,1) = StartPos(j,1) - winRect(3)- blobsize;
                    end
                    if StartPos(j,1) < winRect(1)- blobsize
                        StartPos(j,1) = StartPos(j,1) + winRect(3) + blobsize;
                    end
                    if StartPos(j,2) > winRect(4)
                        StartPos(j,2) = StartPos(j,2) - winRect(4) - blobsize;
                    end
                    if StartPos(j,2) < winRect(2) - blobsize
                        StartPos(j,2) = StartPos(j,2) + winRect(4) + blobsize;
                    end
                end
            end
...
%                 Screen('FillArc', win, 0, [Start End], 0, 360);
                   Screen('DrawTexture', win, owl, [], [Start End], atan2(NewDirection(j,2),NewDirection(j,1))*180/pi+180)
            end
